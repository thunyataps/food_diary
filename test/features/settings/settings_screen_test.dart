...
void main() {
  testWidgets('prefills fields from initialGoals and saves edited values', (
    ...
    await tester.enterText(
      find.byKey(const Key('calories_goal_field')),
      '1800',
    );
  ...
  testWidgets('starts with empty fields when there are no initial goals', (
    tester,
  ) async {
    await tester.pumpWidget(
      MaterialApp(
  ...
  testWidgets('shows an error and does not save when a field is not a number', (
    tester,
  ) async {
    var saveCalls = 0;
    await tester.pumpWidget(
