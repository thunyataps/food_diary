...
void main() {
  test(
    'compressFoodPhoto returns compressed bytes with jpeg mimeType',
    () async {
      final compressor = ImageCompressor(
        compressFn: (path, minWidth, minHeight, quality) async =>
  ...
  test('compressFoodPhoto throws when compression returns null', () async {
    ...
    expect(
      () => compressor.compressFoodPhoto(File('fake.jpg')),
      throwsException,
    );
  });
}
